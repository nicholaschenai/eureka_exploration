...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Maintain upright position (bonus for being close to upright)
    # Assuming optimal height for upright is 1.0
    target_z = 1.0
    z_deviation = torch.abs(torso_position[:, 2] - target_z)
    upright_bonus = 1.0 - z_deviation  # Linear approach to enhance variation
    
    # Penalize deviations from stable orientations (e.g., falling)
    stability_penalty_weight = 1.0
    stability_penalty = stability_penalty_weight * z_deviation.pow(2)  # Quadratic for harsher penalties on larger deviations
    ...
    speed_temp = 0.1
    upright_temp = 5.0  # Increased to ensure effectively translates small deviations
