...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Speed reward: Project velocity onto the target direction
    speed_along_target = (velocity * normalized_to_target).sum(dim=-1)

    # Upright reward: Reward maintaining a target z position
    target_z = 1.0
    z_deviation = torch.abs(torso_position[:, 2] - target_z)
    upright_bonus = 1.0 - z_deviation  # Linearly discourage deviation
    
    # Adjust upright reward to increase its variability and sensitivity
    upright_bonus_adjustment = upright_bonus * (2.0 - speed_along_target.clamp(min=0))  # Consider speed influence
    
    # Penalize deviations for stability
    stability_penalty_weight = 0.5
    stability_penalty = stability_penalty_weight * z_deviation

    # Calculate overall reward with transformations
    ...
    upright_reward = upright_bonus_weight * torch.exp(upright_temp * upright_bonus_adjustment)
